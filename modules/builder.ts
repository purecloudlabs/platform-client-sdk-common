...
import { resourcepaths, version, PureCloud, ApiVersionData}  from './types/builderTypes'
...
import SwaggerDiff from './swaggerDiff';
import GitModule from './gitModule';
import Proxy from './proxy-npm';
import Zip from './zip';
...
// import * as  swaggerDiff from './swaggerDiff';
// import * as git from './gitModule';
// import * as  zip from './zip';
// import * as log from './logger';
// import * as proxy from './proxy-npm';
...
export class Builder {
	...
	constructor(configPath: string, localConfigPath: string) {
		try {
	...
	try {
		...
		git
			...
			.then(function (apiVersionData: string) {
				...
				console.log(_this.apiVersionData );
				  const data: Data = {
...
function applyOverrides(original: { [key: string]: any }, overrides : { [key: string]: any }) {
...
function executeScript(script) {
	...
	let completedMessage = `Script completed with return code ${code} in ${measureDurationFrom(startTime)}`;
	if (bufferCode !== 0) {
...
function getScriptPath(script: any) {

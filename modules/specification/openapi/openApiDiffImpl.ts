...
import { OpenApiSpec, OpenApiInfo, Path, RestResponse, ItemsType, Property, HttpMethod, valueTypes, Parameter } from '../../types/openapiSpec.js';
...
export class OpenApiDiffImpl {

	changes: Changes = {};
	changeCount: number = 0;
	newApiVersion: string = "";
	openapiVersion: string = "";
	openapiInfo: OpenApiInfo = {} as OpenApiInfo;
	oldOpenApi: OpenApiSpec = {} as OpenApiSpec;
	newOpenApi: OpenApiSpec = {} as OpenApiSpec;
	...
	public diff(oldOpenApi: OpenApiSpec, newOpenApi: OpenApiSpec): void {
		...
		this.checkOperations(oldOpenApi, newOpenApi);
		log.info('Checking models for changes');
		this.checkModels(oldOpenApi, newOpenApi);

		log.info(`OpenApiSpec diff implementation complete. Found ${this.changeCount} changes.`);
	};

	private checkOperations(oldOpenApi: OpenApiSpec, newOpenApi: OpenApiSpec) {
		let diffImpl = this as OpenApiDiffImpl;
		if (!oldOpenApi) {
						...
						if (!oldOperation) {
							...
							_.forEach(newOperation.responses, function (newResponse, newResponseCode) {
								...
								if (!oldResponse) {
									...
									// JSM TODO
									// Also need to loop on contentTypes to check for response content type added, removed, updated
									checkForChange(
										diffImpl,
										operationMethodAndPath,
										newResponseCode,
									...
									let contentTypes = Object.keys(oldResponse.content ?? {});
									let firstContentType = contentTypes.length > 0 ? contentTypes[0] : 'application/json';
									// JSM TODO - reactiver
									// checkForChange(
									// 	operationMethodAndPath,
									// 	newResponseCode,
									...
									// 	IMPACT_MAJOR,
									// 	'$ref',
									// 	oldResponse.content[firstContentType].schema,
									// 	newResponse.content[firstContentType].schema,
									// 	`Response ${newResponseCode} type was changed from ${getSchemaType(oldResponse.content[firstContentType].schema)} to ${getSchemaType(
									// 		newResponse.content[firstContentType].schema
									// 	)}`
									// );
								}
	...
	private checkModels(oldOpenApi: OpenApiSpec, newOpenApi: OpenApiSpec) {
		let diffImpl = this as OpenApiDiffImpl;
		if (!oldOpenApi) {
			...
			if (!oldModel) {
				...
				_.forEach(newModel.properties, function (newProperty, propertyKey) {
					...
					if (!oldProperty) {
						...
						if (!type) type = newProperty['$ref'] ? newProperty['$ref'].replace('#/components/schemas/', '') as ItemsType : undefined;
...
function addChange(diffImpl: OpenApiDiffImpl, id: string, key: string, location: string, impact: string, oldValue: valueTypes | undefined, newValue: valueTypes | undefined, description: string | undefined) {
...
function checkForChange(diffImpl: OpenApiDiffImpl, id: string, key: string | undefined, location: string, impact: string, property: string | undefined, oldObject: any, newObject: any, description: string | undefined) {
...
function getSchemaType(schema: Property): string {
	...
	if (schema && schema.type) {
		if (schema.type.toLowerCase() == 'array' && schema.items) {
			if (schema.items['$ref']) {
				const arrayType = `${schema.items['$ref'].replace('#/components/schemas/', '')}[]`;

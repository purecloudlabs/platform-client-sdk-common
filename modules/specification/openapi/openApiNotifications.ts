...
import { ItemsType, Format, OpenApiSpec } from '../../types/openapiSpec.js';
...
import { mergeSpecificationPreprocessingCfg } from './openApiUtils.js';

const NOTIFICATION_ID_REGEX = /^urn:jsonschema:(.+):v2:(.+)$/i;

//#region Notifications Preprocessing

export async function openapiAddNotifications(gcConfig: PureCloud, openapi: OpenApiSpec, overrideCfg: SpecificationPreprocessing | null): Promise<string> {
		...
		try {
			...
			gcLoginClientCredentialsGrant(gcConfig.environment, gcConfig.clientId, gcConfig.clientSecret)
				...
				.then((notifications: AvailableTopicEntityListing) => {
					...
					// Process schemas and transform to openapi components schemas
					let openapi_notifications: OpenApiSpec = {
						paths: {},
						components: {
							schemas: {},
							securitySchemes: {}
					...
					_.forEach(notifications.entities, (entity) => {
						...
						openapiExtractSchemas(openapi_notifications, cfg, entity.schema);
						openapi_notifications.components.schemas[schemaName] = JSON.parse(JSON.stringify(entity.schema));
					});

					// Remove type: any in Notification Topics
					openapiProcessAnyTypes(openapi_notifications, cfg);

					// Merge Notification Topics with OpenApi
					if (openapi_notifications.components && openapi_notifications.components.schemas) {
						for (let modelName in openapi_notifications.components.schemas) {
							if (!openapi.components.schemas[modelName]) openapi.components.schemas[modelName] = openapi_notifications.components.schemas[modelName];
						}
					}
...
function openapiProcessAnyTypes(openapi: OpenApiSpec, cfg: SpecificationPreprocessing) {
	if (cfg.notifications.replaceTypeAny !== true) return;

	if (!openapi.components || !openapi.components.schemas) return;

	const keys = Object.keys(openapi.components.schemas);
	keys.forEach((key, index) => {
		let obj = openapi.components.schemas[key].properties;
		if (obj) {
...
function openapiExtractSchemas(openapi: OpenApiSpec, cfg: SpecificationPreprocessing, entity: { [key: string]: any }) {
	try {
		_.forOwn(entity, (property, key) => {
			// Rewrite URN refs to JSON refs
			if (key == '$ref' && !property.startsWith('#')) {
				entity[key] = '#/components/schemas/' + getNotificationClassName(property);
			}
			...
			openapiExtractSchemas(openapi, cfg, property);
			...
			if (property.id && typeof property.id === 'string') {
				...
				// Store definition
				openapi.components.schemas[className] = JSON.parse(JSON.stringify(property));
